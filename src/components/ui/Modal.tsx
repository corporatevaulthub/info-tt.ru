...
const Modal = ({ children }) => {
  ...
  const sendEmail = (formData) => {
    ...
    emailjs
      .sendForm(
        "service_s3o21hb",   // Замените на ваш Service ID
        "x7xNkcxUMtjXOmzEK",  // Замените на ваш Template ID
        form.current,
        "x7xNkcxUMtjXOmzEK"    // Замените на ваш Public Key
      )
      .then(
        (result) => {
          setLoading(false);
                ...
                <Dialog.Panel className="relative w-full max-w-md transform overflow-hidden bg-white px-6 py-10 text-left align-middle shadow-xl transition-all">
                  ...
                  <form ref={form} onSubmit={handleSubmit(sendEmail)}>
                    <fieldset className="mb-4 flex w-full flex-col items-start gap-3">
                      <input
                        className={`input h-[43px] w-full bg-gray-50 ${
                    ...
                    <label className="mb-5 flex cursor-pointer items-center gap-2 text-[#2E3037]">
                      <input
                        ...
                        className="form-checkbox rounded border-[#D5DAE3] text-primary focus:ring-0"
                      />
                    ...
                    <button
                      className="button h-[43px] w-full justify-center bg-accent px-[43px] hover:bg-accent/90 disabled:pointer-events-none disabled:bg-dark"
                      disabled={loading}
                    >
